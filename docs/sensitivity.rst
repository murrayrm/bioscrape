...
role as a precursor to :doc:`inference`.  Not every parameter that
appears in a model can actually be estimated from a given measurement
-- if a species' trajectory barely changes as a parameter is varied,
no amount of data on that trajectory will pin the parameter down, and
running expensive MCMC sampling to try is wasted effort.  Checking
which parameters a measured output is sensitive to, before fitting,
tells you which parameters are worth estimating from that data and
which should be fixed or determined separately.  For a model with
several measured outputs, this can be done one output at a time: fit
the parameters the first output is sensitive to, fix them, then use
the sensitivities of a second output (holding the first parameters
fixed) to decide what to fit next, and so on [Pan+23d]_.
...
directly against an analytical result, the same way a stochastic
simulation was checked against a known distribution in
:doc:`simulation`.  For the birth-death model built in :doc:`model`,
`X` reaches a deterministic steady state of `k_prod` / `k_deg`, and its
steady-state sensitivities to the two rate parameters have simple
closed forms: :math:`\partial X/\partial k_{prod} = 1/k_{deg}` and
:math:`\partial X/\partial k_{deg} = -k_{prod}/k_{deg}^2`.  Running
`~py_sensitivity_analysis` and letting the trajectory settle shows the
computed sensitivities converging to exactly these values:
...
*Sensitivity Analysis using Bioscrape* notebook in :doc:`tutorials`.
...
inference (:doc:`inference`) was then run on only those parameters,
one measured species at a time.
